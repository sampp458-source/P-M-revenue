...
export function DashboardPeriodFilters({ period, range, unitName, compare, onPeriod, onCustom, onMovePeriod, onCompare }: { period: DashboardPeriod; range: DashboardDateRange; unitName: string; compare: DashboardCompare; onPeriod: (period: DashboardPeriod) => void; onCustom: (range: DashboardDateRange) => void; onMovePeriod: (direction: number) => void; onCompare: (compare: DashboardCompare) => void }) {
  return (
    <div className="dashboard-period-surface dashboard-period-toolbar pm-d-period-control mb-4">
...
export function SalesHeatmapCalendar({ month, activeRange, data, totalData, unitName, themeCode, today, selectedDate, hideAmounts = false, onMonth, onSelect }: { month: string; activeRange: DashboardDateRange; data: DailyRevenue[]; totalData: DailyRevenue[]; unitName: string; themeCode: DashboardThemeCode; today: string; selectedDate: string; hideAmounts?: boolean; onMonth: (month: string) => void; onSelect: (date: string) => void }) {
  ...
  return (
    <Card className="dashboard-surface dashboard-calendar-surface pm-d-workspace p-3.5 shadow-none sm:p-6">
      <div className="mb-2.5 flex min-h-8 items-center justify-between gap-3 sm:mb-3">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          <h2 className="text-[clamp(1.05rem,1.7vw,1.2rem)] font-bold tracking-[-0.025em] text-text-primary">{monthTitle}</h2>
          <span className="dashboard-theme-badge inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-bold">
            <span className="dashboard-theme-dot h-1.5 w-1.5 rounded-full" aria-hidden="true" />
            {theme.label}
          </span>
        </div>
        <div className="flex shrink-0 items-center gap-0.5">
          <Button type="button" variant="ghost" className="h-8 min-h-8 px-2" aria-label="이전 달" onClick={() => onMonth(moveMonth(month, -1))}>
            <ChevronLeft size={17} />
          </Button>
          <Button type="button" variant="ghost" className="h-8 min-h-8 px-2.5 text-xs font-semibold" onClick={() => onMonth(today.slice(0, 7))}>
            오늘
          </Button>
          <Button type="button" variant="ghost" className="h-8 min-h-8 px-2" aria-label="다음 달" onClick={() => onMonth(moveMonth(month, 1))}>
