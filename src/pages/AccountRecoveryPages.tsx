...
function AccountShell({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <main className="pm-access-v1 pm-access-v2 pm-account-recovery flex min-h-screen items-center justify-center bg-[#f0f3f1] p-6"><Card className="w-full max-w-md p-6 sm:p-8"><h1 className="text-2xl font-bold">{title}</h1><p className="mt-2 text-sm leading-6 text-slate-500">{description}</p>{children}</Card></main>;
}
