...
describe("Journal image export", () => {
  ...
  it.each([
    ...
    const toBlob = vi.fn((callback: BlobCallback, type?: string, requestedQuality?: number) => {
      expect(type).toBe(mimeType);
      expect(requestedQuality).toBe(quality);
      callback(new Blob([format], { type }));
    });
    const canvas = { width: 1080, height: 1440, toBlob } as unknown as HTMLCanvasElement;
  ...
  it("fails closed when the rasterizer returns the wrong dimensions", async () => {
    ...
    await expect(exportJournalImage(root, viewModel, "png")).rejects.toThrow("JOURNAL_EXPORT_SIZE_MISMATCH");
