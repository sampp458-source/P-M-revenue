...
export function HotelDateTransition({date, ready, error, onRetry, retryInFlight = false, children}: {
  ...
  useLayoutEffect(()=>{
    if(blocked && presentationRef.current?.contains(document.activeElement))statusRef.current?.focus({preventScroll:true});
  },[blocked]);
  ...
  return <section aria-label="호텔 날짜별 화면" aria-busy={blocked && (!error || retryInFlight)}>
    <div ref={statusRef} tabIndex={-1} className="mb-2 flex min-h-6 flex-wrap items-center gap-2 text-xs text-text-secondary" role={error && !retryInFlight ? 'alert' : 'status'} aria-live="polite">
      {blocked ? <>
    ...
    <div ref={presentationRef} inert={blocked} onClickCapture={stop} onKeyDownCapture={stop} onPointerDownCapture={stop} onPointerUpCapture={stop} onDragStartCapture={stop} onDragOverCapture={stop} onDropCapture={stop} onSubmitCapture={stop}
