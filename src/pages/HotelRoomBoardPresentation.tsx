...
export function RoomBoardCellFrame({mobile=false,className,...props}:HTMLAttributes<HTMLDivElement>&{mobile?:boolean}) {
 return <div {...props} className={cn('relative overflow-visible rounded-xl border transition-[transform,box-shadow,border-color,background-color,opacity] duration-150 ease-out motion-reduce:transition-none',mobile?'min-h-[4.5rem] p-2.5':'min-h-[5.5rem] p-1.5',className)}/>;
}
