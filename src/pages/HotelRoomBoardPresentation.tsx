...
export function RoomBoardMobileGroup({type,summary,expanded,onToggle,children}:{type:string;summary:ReactNode;expanded:boolean;onToggle:()=>void;children:ReactNode}) {
 return <section data-room-group={type} aria-label={`${type} 모바일 Room Board`} className="overflow-hidden rounded-2xl border border-border bg-surface"><button type="button" aria-expanded={expanded} onClick={onToggle} className="flex min-h-14 w-full items-center justify-between gap-3 px-4 py-3 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary"><span><strong className="block text-base text-text-primary">{type}</strong>{summary!=null?<span className="mt-0.5 block text-xs font-semibold text-text-secondary">{summary}</span>:null}</span><ChevronDown className={cn('shrink-0 transition-transform duration-150 motion-reduce:transition-none',expanded&&'rotate-180')} size={20}/></button><div hidden={!expanded} className="border-t border-border px-3 py-3">{children}</div></section>;
}
