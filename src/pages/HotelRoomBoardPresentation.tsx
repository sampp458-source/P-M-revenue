...
export function RoomBoardDesktopGroup({type,summary,badge,children}:{type:string;summary:ReactNode;badge?:ReactNode;children:ReactNode}) {
 return <section aria-label={`${type} Room Board`}><div className="mb-3 flex items-end justify-between gap-3 border-b border-border pb-2"><div><h3 className="text-base font-extrabold text-text-primary">{type}</h3><p className="mt-0.5 text-xs font-semibold text-text-secondary">{summary}</p></div>{badge}</div><div className={cn('grid gap-3',type==='DELUXE'?'min-w-[720px] grid-cols-6':'min-w-[600px] grid-cols-5')}>{children}</div></section>;
}
export function RoomBoardMobileGroup({type,summary,expanded,onToggle,children}:{type:string;summary:ReactNode;expanded:boolean;onToggle:()=>void;children:ReactNode}) {
 return <section aria-label={`${type} 모바일 Room Board`} className="overflow-hidden rounded-2xl border border-border bg-surface"><button type="button" aria-expanded={expanded} onClick={onToggle} className="flex min-h-14 w-full items-center justify-between gap-3 px-4 py-3 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary"><span><strong className="block text-base text-text-primary">{type}</strong><span className="mt-0.5 block text-xs font-semibold text-text-secondary">{summary}</span></span><ChevronDown className={cn('shrink-0 transition-transform',expanded&&'rotate-180')} size={20}/></button>{expanded?<div className="border-t border-border px-3 py-3">{children}</div>:null}</section>;
}
