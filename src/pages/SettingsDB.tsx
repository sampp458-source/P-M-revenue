...
export function SettingsPage() {
  ...
  return <section className="pm-design-v1 pm-settings-v1 pm-target-settings-v1">
...
function TargetCard({ title, description, value, editable, status, onChange, onSave }: { title: string; description: string; value: string; editable: boolean; status: SaveStatus; onChange: (value: string) => void; onSave: () => void }) { return <Card className="target-overall p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-end"><div className="flex-1"><h2 className="font-bold">{title}</h2><p className="mt-1 text-sm text-slate-500">{description}</p><div className="mt-4 max-w-sm"><MoneyField targetKey="overall" value={value} editable={editable} onChange={onChange} onEnter={onSave} /></div></div>{editable && <div className="flex items-center gap-3"><StatusText status={status} /><Button type="button" disabled={status === "saving"} onClick={onSave}>{status === "saving" ? "저장 중..." : "전체 목표 저장"}</Button></div>}</div></Card>; }
function TargetRow({ targetKey, name, value, editable, status, onChange, onSave }: { targetKey: string; name: string; value: string; editable: boolean; status: SaveStatus; onChange: (value: string) => void; onSave: () => void }) { return <div className="target-unit-row grid gap-3 px-5 py-4 sm:grid-cols-[140px_1fr_auto] sm:items-center"><div><p className="font-semibold">{name}</p><StatusText status={status} /></div><MoneyField targetKey={targetKey} value={value} editable={editable} onChange={onChange} onEnter={onSave} />{editable && <Button type="button" variant="secondary" disabled={status === "saving"} onClick={onSave}>{status === "saving" ? "저장 중..." : "저장"}</Button>}</div>; }
