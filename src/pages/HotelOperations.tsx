...
function StayDetailModal({ open, stay, selectedDate, loading, creatorName, sharedOccupancy, canMergeSharedRoom, onClose, onEdit, onAssign, onReassign, onMove, onUnassign, onCheckIn, onCheckOut, onChangePlannedCheckout, onCancel, onMergeSharedRoom }: { open: boolean; stay: HotelStay | null; selectedDate: string; loading: boolean; creatorName: string | null; sharedOccupancy: SharedHotelOccupancy | null; canMergeSharedRoom: boolean; onClose: () => void; onEdit: () => void; onAssign: () => void; onReassign: () => void; onMove: () => void; onUnassign: () => void; onCheckIn: () => void; onCheckOut: () => void; onChangePlannedCheckout: () => void; onCancel: () => void; onMergeSharedRoom: () => void }) {
  ...
  return (
    <Modal
      open={open}
      title="호텔 예약 상세"
      description={stay ? `${stay.dogName} · ${sharedOccupancy?.roomName ?? allocation?.roomName ?? "호실 미배정"}` : undefined}
      onClose={onClose}
      size="large"
      resetKey={stay?.id}
    >
      {loading || !stay ? <LoadingState /> : <div className="space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><h3 className="text-xl font-bold text-text-primary">{hotelStayDayTitle(stay, selectedDate)}</h3><p className="mt-1 text-sm text-text-secondary">🐶 {stay.dogName} · {stay.customerName ?? "보호자 미등록"}</p><div className="mt-2 flex flex-wrap gap-1.5">{unspecified.checkInTime ? <Badge tone="amber">입실시간 미정</Badge> : null}{unspecified.checkOutTime ? <Badge tone="amber">퇴실시간 미정</Badge> : null}{unspecified.roomType ? <Badge tone="amber">객실 미정</Badge> : null}</div></div>
        ...
        <ResponsiveActionGroup
          ...
          secondary={<>{!stay.checkedInAt ? <Button type="button" variant="secondary" onClick={onEdit}>예약 수정</Button> : null}{canChangeCheckedInHotelPlannedCheckout(stay) ? <Button type="button" variant="secondary" onClick={onChangePlannedCheckout}>퇴실 예정 변경</Button> : null}{canMergeSharedRoom ? <Button type="button" variant="secondary" onClick={onMergeSharedRoom}><BedDouble size={16} /> 같은 방 투숙</Button> : null}{!stay.checkedInAt && !allocation && !unspecified.roomType ? <Button type="button" variant="secondary" onClick={onAssign}>호실 배정</Button> : null}{!stay.checkedInAt && allocation ? <><Button type="button" variant="secondary" onClick={onReassign}>호실 재배정</Button><Button type="button" variant="secondary" onClick={onUnassign}>배정 해제</Button></> : null}{stay.checkedInAt && !stay.checkedOutAt ? <Button type="button" variant="secondary" onClick={onMove}><MoveRight size={16} /> 객실 이동</Button> : null}</>}
