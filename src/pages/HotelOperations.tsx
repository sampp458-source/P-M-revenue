...
import {
  ...
  fetchHotelEventRoomProjections,
  type HotelEventRoomProjections,
  fetchHotelStay,
...
function plannedCheckoutErrorToast(error: unknown): HotelToast {
  if (error instanceof HotelOperationsRepositoryError) {
    if (error.kind === "room_conflict") {
      return {
        title: "퇴실 예정일을 변경할 수 없습니다",
        message: "해당 기간에 이 객실의 다음 예약이 있어 연장할 수 없습니다.",
        tone: "error",
      };
    }
    if (error.kind === "capacity_conflict") {
      return {
        title: "퇴실 예정일을 변경할 수 없습니다",
        message: "해당 기간에 예약 가능한 객실 수가 부족합니다.",
        tone: "error",
      };
    }
    if (error.kind === "conflict") {
      return {
        title: "최신 상태를 다시 확인해 주세요",
        message: "다른 사용자가 먼저 변경했습니다. 최신 퇴실 예정 정보를 불러왔습니다.",
        tone: "error",
      };
    }
  }
...
export function HotelOperationsPage() {
  ...
  const changePlannedCheckout = async (
    stay: HotelStay,
    checkOutDate: string,
    ...
    setProcessing(true);
    try {
      const result = await updateCheckedInHotelPlannedCheckout(
        stay.id,
        stay.version,
        checkOutDate,
        checkOutTime,
      ...
      if (
        ...
        const latestStay = await fetchHotelStay(stay.id).catch(() => null);
        if (latestStay) setDetail(latestStay);
  ...
  const confirmSharedRoomUnassign = () => {
    if (isPast) return;
    if (!pendingSharedRoomUnassign || processing) return;
    ...
    void action(
      ...
      pendingSharedRoomUnassign.requestId,
    ).then(async () => {
      setPendingSharedRoomUnassign(null);
      ...
      setToast({
        tone: "success",
        message: "객실 배정을 해제했습니다. 예약은 미배정 상태로 유지됩니다.",
      });
    }).catch(async (error) => {
      await loadSnapshot(selectedDate).catch(() => undefined);
      setToast({ tone: "error", message: sharedHotelRoomErrorMessage(error) });
  ...
  return (
    <div className="pm-design-v1 pm-hotel-v1 pm-design-v2 pm-hotel-v2 pm-design-d pm-d-page">
      ...
      />
      <ConfirmModal
        open={!isPast && cancelSharedGroupId !== null}
          ...
          {filteredStays.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {filteredStays.map((stay) => (
                <StayRow eventRoomProjections={eventRoomProjections} key={stay.id} stay={stay} selectedDate={selectedDate} onClick={() => void openStay(stay.id)} />
              ))}
            </div>
          )}
      ...
      <ConfirmModal
        ...
        processing={processing}
        onClose={() => {
          if (!processing) setPendingSharedRoomUnassign(null);
        }}
        onConfirm={confirmSharedRoomUnassign}
      />
      </> : null}
...
export function StayRow({ stay, selectedDate, onClick, eventRoomProjections }: { eventRoomProjections?: HotelEventRoomProjections; stay: HotelStay; selectedDate: string; onClick: () => void }) {
  const eventRoomSummary = hotelStayEventRoomSummary(stay, selectedDate, eventRoomProjections);
    ...
    <button type="button" onClick={onClick} className="grid w-full gap-3 px-5 py-4 text-left transition hover:bg-primary-subtle focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-6">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <b className="truncate text-sm text-text-primary">{eventRoomSummary ? `${stay.dogName} · 호텔링 · ${dayPhase}` : hotelStayDayTitle(stay, selectedDate)}</b>
          ...
          {!eventRoomSummary && unspecified.roomType ? <Badge tone="amber">객실 미정</Badge> : null}
        </div>
...
export function StayDetailModal({ readOnly = false, currentRoomLabel, eventRoomProjections, open, stay, selectedDate, loading, creatorName, sharedOccupancy, canMergeSharedRoom, operationRole, onClose, onEdit, onAssign, onReassign, onMove, onUnassign, onCheckIn, onCheckOut, onReverseCheckIn, onChangePlannedCheckout, onCancel, onMergeSharedRoom }: { readOnly?: boolean; currentRoomLabel?: string; eventRoomProjections?: HotelEventRoomProjections; open: boolean; stay: HotelStay | null; selectedDate: string; loading: boolean; creatorName: string | null; sharedOccupancy: SharedHotelOccupancy | null; canMergeSharedRoom: boolean; operationRole: OperationRole | null; onClose: () => void; onEdit: () => void; onAssign: () => void; onReassign: () => void; onMove: () => void; onUnassign: () => void; onCheckIn: () => void; onCheckOut: () => void; onReverseCheckIn: () => void; onChangePlannedCheckout: () => void; onCancel: () => void; onMergeSharedRoom: () => void }) {
  ...
  return (
    <Modal
      open={open}
      title="호텔 예약 상세"
      description={stay ? `${stay.dogName} · 호텔 예약` : undefined}
      onClose={onClose}
      size="large"
      resetKey={stay?.id}
    >
      {loading || !stay ? <LoadingState /> : <div className="space-y-6">
