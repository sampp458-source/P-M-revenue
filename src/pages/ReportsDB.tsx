...
export function ReportsPage() {
  ...
  return <section className="pm-finance-v1 pm-reports-v1 pm-design-v2 pm-finance-v2 pm-design-d pm-d-page pm-d-rollout3">
    ...
    <div className="finance-divisions mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{report.divisions.map((row) => <Metric key={row.name} label={`${row.name} 판매금액`} value={won(row.value)} detail={`실수납 ${won(row.paidAmount)} · 환불 ${won(row.refundAmount)}`} />)}</div>
    <Card className="finance-methods mt-4 p-5"><h2 className="font-bold">결제수단별 수납</h2><p className="mt-1 text-xs text-slate-500">결제일 기준 유효 결제원장 합계</p>{report.paymentMethods.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{report.paymentMethods.map((row) => <div key={row.method} className="rounded-xl border border-border bg-surface-secondary p-4"><span className="text-xs text-text-muted">{row.name}</span><strong className="mt-1 block text-lg text-text-primary tabular-nums">{won(row.value)}</strong></div>)}</div> : <EmptyState title="선택 월의 수납 내역이 없습니다." />}</Card>
...
function Metric({ label, value, detail }: { label: string; value: string; detail?: string }) { return <Card className="finance-metric p-5"><p className="text-sm font-medium text-slate-500">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p>{detail && <p className="mt-2 text-xs text-slate-500">{detail}</p>}</Card>; }
...
function Chart({ title, children }: { title: string; children: React.ReactNode }) { return <Card className="finance-chart p-5"><h2 className="mb-4 font-bold">{title}</h2><div className="min-w-0">{children}</div></Card>; }
function Rank({ title, rows, empty }: { title: string; rows: { name: string; value: number; count: number }[]; empty: string }) { return <Card className="finance-list overflow-hidden"><h2 className="border-b px-5 py-4 font-bold">{title}</h2>{rows.length ? <div className="divide-y">{rows.map((row, index) => <div key={`${row.name}-${index}`} className="finance-rank-row flex items-center gap-3 px-5 py-3 text-sm"><span className="flex h-7 w-7 items-center justify-center rounded bg-slate-100 text-xs font-bold">{index + 1}</span><span className="min-w-0 flex-1 truncate font-medium">{row.name}</span><span className="text-xs text-slate-500">{row.count}건</span><b>{won(row.value)}</b></div>)}</div> : <EmptyState title={empty} />}</Card>; }
...
function EventList({ title, rows }: { title: string; rows: Array<HistoryEvent & { sale: ReportSale | undefined; actor: string }> }) { return <Card className="finance-list overflow-hidden"><h2 className="border-b px-5 py-4 font-bold">{title}</h2>{rows.length ? <div className="divide-y">{rows.map((row) => { const refundDelta = Math.max(0, refundValue(row.changedData) - refundValue(row.previousData)); return <div key={row.id} className="finance-event-row p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><StatusBadge status={row.action as "partial_refund" | "full_refund" | "cancelled"} /><strong className="text-sm">{row.sale?.productName || "상품 정보 없음"}</strong></div><span className="text-xs text-slate-500">{new Date(row.createdAt).toLocaleString("ko-KR")}</span></div><p className="mt-2 text-sm text-slate-600">{row.sale?.businessUnitName || "-"} · 처리자 {row.actor}{row.action !== "cancelled" ? ` · 환불 ${won(refundDelta)}` : ""}</p></div>; })}</div> : <EmptyState title="표시할 내역이 없습니다." />}</Card>; }
