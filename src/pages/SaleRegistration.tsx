...
const recentSaleFields = "id, sale_date, created_at, customer_id, customer_name, dog_id, dog_name, business_unit_id, business_unit_name, product_id, product_name, paid_amount, net_amount, status, staff_name";
...
export function SaleFormPage() {
  ...
  const [duplicateSale, setDuplicateSale] = useState<RecentSale | null>(null);
  const [form, setForm] = useState({
    saleDate: today(), businessUnitId: stored(lastBusinessUnitKey), customerId: "", dogId: "", categoryId: "", productId: "",
    originalAmount: 0, discountAmount: 0, paidAmount: 0, refundAmount: 0, outstandingAmount: 0,
    paymentMethod: "card", customerType: "new", staffId: stored(lastStaffKey) || profile?.id || "", memo: "",
  });
  ...
  const loadOptions = useCallback(async () => {
    ...
    const [customersResult, dogsResult, categoriesResult, productsResult, staffResult, salesResult] = await Promise.all([
      ...
      supabase.from("dogs").select("id, name, breed, customer_id, customers(name, phone)").eq("is_active", true).order("name"),
    ...
    setDogs((dogsResult.data ?? []).map((dog) => {
      ...
      return { id: dog.id, name: dog.name, breed: dog.breed, customerId: dog.customer_id, customerName: customer?.name ?? null, customerPhone: customer?.phone ?? null };
    }));
    ...
    setForm((current) => ({
      ...current,
      businessUnitId: businessUnits.some((unit) => unit.id === current.businessUnitId) ? current.businessUnitId : "",
      staffId: staffRows.some((item) => item.id === current.staffId) ? current.staffId : profile?.id ?? staffRows[0]?.id ?? "",
    }));
    setLoading(false);
  }, [businessUnits, profile?.id]);

  useEffect(() => { void loadOptions(); }, [loadOptions]);
  useEffect(() => { if (form.businessUnitId) localStorage.setItem(lastBusinessUnitKey, form.businessUnitId); }, [form.businessUnitId]);
  useEffect(() => { if (form.staffId) localStorage.setItem(lastStaffKey, form.staffId); }, [form.staffId]);
  ...
  const searchResults = useMemo<PartySearchResult[]>(() => {
    ...
    dogs.forEach((dog) => {
      ...
      let score = 99;
      if (dogName === query) score = 0;
      else if (dogName.startsWith(query)) score = 1;
      else if (customerName === query) score = 2;
      else if (customerName.includes(query)) score = 3;
      else if (phoneQuery.length >= 3 && phone.includes(phoneQuery)) score = 4;
      else if (dogName.includes(query)) score = 4;
      if (score < 99) results.push({ key: `dog:${dog.id}`, customerId: dog.customerId, dogId: dog.id, dogName: dog.name, breed: dog.breed, customerName: dog.customerName, customerPhone: dog.customerPhone, dogNames: dog.customerId ? (dogsByCustomer.get(dog.customerId) ?? []).map((item) => item.name) : [dog.name], score });
    });
    customers.forEach((customer) => {
      ...
      const score = name === query ? 2 : name.includes(query) ? 3 : phoneQuery.length >= 3 && phone.includes(phoneQuery) ? 4 : 99;
      if (score < 99) results.push({ key: `customer:${customer.id}`, customerId: customer.id, dogId: null, dogName: null, breed: null, customerName: customer.name, customerPhone: customer.phone, dogNames: customerDogs.map((item) => item.name), score });
    });
  ...
  const resetAfterSave = () => {
    setForm((current) => ({ ...current, saleDate: today(), customerId: "", dogId: "", categoryId: "", productId: "", originalAmount: 0, discountAmount: 0, paidAmount: 0, refundAmount: 0, outstandingAmount: 0, customerType: "new", memo: "" }));
  ...
  const persistSale = async () => {
    ...
    setNotice("매출을 저장했습니다. 다음 매출을 이어서 등록할 수 있습니다.");
    resetAfterSave();
  };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const duplicateResult = await supabase.from("sales").select(recentSaleFields)
      .eq("business_unit_id", form.businessUnitId).eq("product_id", form.productId).eq("paid_amount", Math.trunc(form.paidAmount))
      .neq("status", "cancelled").gte("created_at", cutoff).or(partyFilter).order("created_at", { ascending: false }).limit(1).maybeSingle();
    ...
    if (duplicateResult.data) { savingRef.current = false; setSaving(false); setDuplicateSale(mapRecentSale(duplicateResult.data as Record<string, unknown>)); return; }
    await persistSale();
  };
  ...
  const submitQuickRegistration = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (first.dog_id && first.dog_name) setDogs((current) => current.some((dog) => dog.id === first.dog_id) ? current : [...current, { id: first.dog_id!, name: first.dog_name!, breed: quickForm.breed.trim() || null, customerId: first.customer_id, customerName: first.customer_name, customerPhone: first.customer_phone }].sort((a, b) => a.name.localeCompare(b.name, "ko")));
  ...
  return <>
    ...
    <form onSubmit={submit} className="mx-auto max-w-5xl pb-24 lg:pb-0">
      <Card className="relative z-10 p-5 sm:p-6">
        ...
        {!form.customerId && !form.dogId ? <>
          <div className="relative">
            <SearchBox inputRef={searchRef} value={search} placeholder="반려견 이름, 보호자 이름 또는 전화번호 검색" autoComplete="off" onFocus={() => setSearchFocused(true)} onBlur={() => setTimeout(() => setSearchFocused(false), 120)} onChange={(event) => setSearch(event.target.value)} onClear={() => setSearch("")} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); const first = searchResults[0]; if (first) selectParty(first.customerId, first.dogId); } }} className="[&_input]:h-13 [&_input]:text-[15px]" />
            {searchFocused && search.trim() && <div className="absolute left-0 right-0 top-full z-30 mt-2 max-h-[420px] overflow-auto rounded-2xl border border-border bg-white p-2 shadow-[var(--pm-shadow-elevated)]">
              {searchResults.length ? <div role="listbox" aria-label="고객과 반려견 검색 결과">{searchResults.map((result) => <button key={result.key} type="button" role="option" onMouseDown={(event) => event.preventDefault()} onClick={() => selectParty(result.customerId, result.dogId)} className="flex w-full items-center gap-3 rounded-xl p-3 text-left transition hover:bg-primary-subtle focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-primary-soft text-primary">{result.dogName ? <Dog size={20} /> : <UserRound size={20} />}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-text-primary">{result.dogName || result.customerName || "이름 미등록"}</strong><span className="mt-0.5 block truncate text-xs text-text-secondary">보호자 {result.customerName || "미등록"} · {result.breed || "견종 미등록"} · {maskedPhone(result.customerPhone)}</span>{result.dogNames.length > 1 && <span className="mt-1 block truncate text-[11px] text-text-muted">연결 반려견: {result.dogNames.join(", ")}</span>}</span></button>)}</div> : <div className="p-5 text-center"><p className="text-sm font-medium text-text-primary">검색 결과가 없습니다.</p><p className="mt-1 text-xs text-text-muted">새 보호자와 반려견을 바로 등록할 수 있습니다.</p><Button type="button" className="mt-4" onMouseDown={(event) => event.preventDefault()} onClick={() => openQuickRegistration()}><Plus size={16} />신규 보호자·반려견 등록</Button></div>}
            </div>}
          </div>
          <div className="mt-4 flex flex-wrap items-center gap-2"><span className="mr-1 text-xs font-semibold text-text-muted">최근 등록</span>{recentParties.length ? recentParties.map((party) => <button key={party.key} type="button" onClick={() => selectParty(party.customerId, party.dogId)} className="inline-flex min-h-10 items-center gap-2 rounded-full border border-border bg-surface px-3 text-xs font-medium text-text-secondary transition hover:border-primary/25 hover:bg-primary-subtle hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"><Dog size={14} />{party.dogName || "(반려견 없음)"} <span className="text-text-muted">({party.customerName || "보호자 미등록"})</span></button>) : <span className="text-xs text-text-muted">최근 등록 고객이 없습니다.</span>}<Button type="button" variant="ghost" className="ml-auto" onClick={() => openQuickRegistration()}><Plus size={16} />신규 등록</Button></div>
        </> : <div className="rounded-2xl border border-primary/20 bg-primary-subtle p-4 sm:p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary text-white"><Dog size={22} /></span><div><strong className="text-lg text-text-primary">{selectedDog?.name || "(반려견 없음)"}</strong><p className="mt-0.5 text-sm text-text-secondary">보호자 {selectedCustomer?.name ?? selectedDog?.customerName ?? "미등록"} · {maskedPhone(selectedCustomer?.phone ?? selectedDog?.customerPhone ?? null)}</p></div></div><div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" onClick={clearParty}>다른 고객 선택</Button>{selectedCustomer && <Button type="button" variant="secondary" onClick={() => openQuickRegistration(selectedCustomer)}><Plus size={16} />신규 반려견 추가</Button>}</div></div>{selectableDogs.length > 0 && <div className="mt-4 border-t border-primary/10 pt-4"><p className="mb-2 text-xs font-semibold text-text-secondary">반려견 선택</p><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setForm((current) => ({ ...current, dogId: "" }))} className={cn("min-h-10 rounded-xl border px-3 text-sm font-medium", !form.dogId ? "border-primary bg-primary text-white" : "border-border bg-white text-text-secondary")}>반려견 없음</button>{selectableDogs.map((dog) => <button type="button" key={dog.id} onClick={() => setForm((current) => ({ ...current, dogId: dog.id }))} className={cn("min-h-10 rounded-xl border px-3 text-sm font-medium", form.dogId === dog.id ? "border-primary bg-primary text-white" : "border-border bg-white text-text-secondary hover:border-primary/25")}>{dog.name}</button>)}</div></div>}</div>}
        {(form.customerId || form.dogId) && <div className="mt-5 border-t border-border pt-5"><div className="mb-3 flex items-center gap-2"><Clock3 size={16} className="text-primary" /><h3 className="text-sm font-semibold text-text-primary">최근 거래</h3><span className="text-xs text-text-muted">최대 5건</span></div>{selectedRecentSales.length ? <div className="grid gap-2 md:grid-cols-2">{selectedRecentSales.map((sale) => <button key={sale.id} type="button" onClick={() => navigate("/sales")} className="flex items-center justify-between gap-3 rounded-xl border border-border p-3 text-left transition hover:border-primary/20 hover:bg-primary-subtle focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"><span className="min-w-0"><strong className="block truncate text-sm text-text-primary">{sale.productName}</strong><span className="mt-1 block text-xs text-text-muted">{sale.saleDate} · {sale.businessUnitName} · {sale.staffName || "담당자 미등록"}</span></span><span className="shrink-0 text-right"><strong className="block text-sm tabular-nums text-text-primary">{won(sale.paidAmount)}</strong><StatusBadge status={sale.status} /></span></button>)}</div> : <p className="rounded-xl bg-surface-secondary p-4 text-sm text-text-muted">최근 거래가 없습니다.</p>}</div>}
      </Card>

      <Card className="mt-4 p-5 sm:p-6"><div className="mb-5"><p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">Step 2</p><h2 className="mt-1 text-lg font-semibold text-text-primary">상품 선택</h2></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Field label="상품" required><Select name="productId" value={form.productId} disabled={saving || !form.categoryId} onChange={(event) => { const product = products.find((item) => item.id === event.target.value); const price = product?.defaultPrice ?? 0; setForm((current) => ({ ...current, productId: event.target.value, originalAmount: price, discountAmount: 0, paidAmount: price, refundAmount: 0, outstandingAmount: 0 })); }}><option value="">상품 선택</option>{filteredProducts.map((item) => <option key={item.id} value={item.id}>{item.name} · {won(item.defaultPrice)}</option>)}</Select></Field>
      </div></Card>
      ...
      <Card className="mt-4 p-5 sm:p-6"><div className="mb-5"><p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">Step 4</p><h2 className="mt-1 text-lg font-semibold text-text-primary">담당자·메모</h2></div><div className="grid gap-5 md:grid-cols-2"><Field label="담당자" required><Select name="staffId" value={form.staffId} disabled={saving} onChange={(event) => setForm((current) => ({ ...current, staffId: event.target.value }))}>{staff.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></Field><div className="md:row-span-2"><Field label="메모"><Textarea name="memo" rows={4} maxLength={500} placeholder="전달할 업무 메모가 있을 때만 입력하세요." value={form.memo} disabled={saving} onChange={(event) => setForm((current) => ({ ...current, memo: event.target.value }))} /></Field></div></div>
        ...
        <div className="mt-6 hidden justify-end gap-2 lg:flex"><Button type="button" variant="secondary" disabled={saving} onClick={resetAll}>입력 초기화</Button><Button className="min-w-32" disabled={saving}>{saving && <LoaderCircle className="animate-spin" size={17} />}{saving ? "저장 중..." : "매출 저장"}</Button></div>
      </Card>
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-border bg-white/95 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur lg:hidden"><div className="mx-auto grid max-w-5xl grid-cols-[auto_1fr] gap-2"><Button type="button" variant="secondary" disabled={saving} onClick={resetAll}>초기화</Button><Button disabled={saving}>{saving && <LoaderCircle className="animate-spin" size={17} />}{saving ? "저장 중..." : `매출 저장 · ${won(Math.max(netAmount, 0))}`}</Button></div></div>
    </form>
    ...
    <Modal open={Boolean(duplicateSale)} onClose={() => { if (!savingRef.current) setDuplicateSale(null); }} title="중복 가능성이 있습니다">
      {duplicateSale && <div><div className="rounded-2xl border border-warning/20 bg-warning-soft p-4"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 shrink-0 text-warning" size={20} /><div><p className="text-sm font-semibold text-text-primary">비슷한 매출이 최근에 등록되었습니다.</p><p className="mt-2 text-sm leading-6 text-text-secondary">{duplicateSale.dogName || "(반려견 없음)"} · {duplicateSale.productName} · {won(duplicateSale.paidAmount)}<br />등록자: {duplicateSale.staffName || "미등록"}</p></div></div></div><div className="mt-5 grid gap-2 sm:grid-cols-3"><Button type="button" variant="secondary" onClick={() => { setDuplicateSale(null); navigate("/sales"); }}><ReceiptText size={16} />기존 내역 보기</Button><Button type="button" variant="secondary" onClick={() => setDuplicateSale(null)}>취소</Button><Button type="button" disabled={saving} onClick={() => { if (savingRef.current) return; savingRef.current = true; setDuplicateSale(null); setSaving(true); void persistSale(); }}>{saving && <LoaderCircle className="animate-spin" size={17} />}그래도 등록</Button></div></div>}
    </Modal>
  </>;
}
