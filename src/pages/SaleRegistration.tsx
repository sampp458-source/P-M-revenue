...
export function SaleFormPage() {
  ...
  const [form, setForm] = useState({
    ...
    quantity: 1, unitPrice: 0, originalAmount: 0, discountAmount: 0, paidAmount: 0, refundAmount: 0, outstandingAmount: 0,
  ...
  const loadOptions = useCallback(async () => {
    ...
    const [customersResult, dogsResult, categoriesResult, productsResult, staffResult, salesResult, pricingSchemaResult] = await Promise.all([
  ...
  const paymentDifference = form.paidAmount - form.originalAmount;
  ...
  const selectProduct = (product: ProductOption) => {
    if (form.productId === product.id) {
      setForm((current) => ({ ...current, productId: "", quantity: 1, unitPrice: 0, originalAmount: 0, discountAmount: 0, paidAmount: 0, refundAmount: 0, outstandingAmount: 0 }));
      setPaidAmountEdited(false);
      return;
    }
  ...
  const updatePricing = (unitPrice: number, quantity: number) => {
    setForm((current) => ({ ...current, ...calculatePricingChange({ unitPrice, quantity, paidAmount: current.paidAmount, paidAmountEdited }) }));
  };

  const useCalculatedPayment = () => {
    setPaidAmountEdited(false);
    setForm((current) => ({ ...current, paidAmount: current.originalAmount }));
  };
  ...
  const clearParty = () => {
    setForm((current) => ({ ...current, customerId: "", dogId: "", ...(!repeatSettings.keepProduct ? { categoryId: "", productId: "", quantity: 1, unitPrice: 0, originalAmount: 0, discountAmount: 0, paidAmount: 0, refundAmount: 0, outstandingAmount: 0 } : {}) }));
  ...
  const resetForSameParty = () => {
    setForm((current) => ({ ...current, categoryId: "", productId: "", quantity: 1, unitPrice: 0, originalAmount: 0, discountAmount: 0, paidAmount: 0, refundAmount: 0, outstandingAmount: 0, memo: "" }));
    setPaidAmountEdited(false);
  ...
  const resetAll = () => {
    setForm((current) => ({ saleDate: today(), businessUnitId: current.businessUnitId, customerId: "", dogId: "", categoryId: "", productId: "", quantity: 1, unitPrice: 0, originalAmount: 0, discountAmount: 0, paidAmount: 0, refundAmount: 0, outstandingAmount: 0, paymentMethod: "card", customerType: "new", staffId: current.staffId, memo: "" }));
    setPaidAmountEdited(false);
    setSaleReference(emptySaleReference()); setCustomerSectionOpen(false);
  ...
  const validate = (formElement: HTMLFormElement) => {
    ...
    if (!isValidPaymentPlan(form)) { setError("할인 후 예정액과 실제 결제·미수금 관계를 확인해 주세요."); setAdvancedOpen(true); focus("outstandingAmount"); return false; }
    if (!pricingSchemaReady && (form.quantity !== 1 || form.paidAmount + form.outstandingAmount > expectedAmount)) { setError("수량 또는 추가요금 저장을 위한 DB 마이그레이션 적용이 필요합니다."); focus("quantity"); return false; }
    return true;
  };
