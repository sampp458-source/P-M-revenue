-- V2-A append-only READ correction. Apply before the unshipped V2-B candidate.
-- No table/trigger/policy changes, lifecycle writes, or business data repair.
BEGIN;

-- Retained-link provenance only. A missing link remains fail-closed, even with
-- complete snapshots: normal physical DELETE is prohibited by the ledger contract.
CREATE FUNCTION public.dog_schedule_audit_resolved_v2a(p_audit jsonb,p_dog_id uuid)
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=public,pg_temp AS $$
DECLARE link_json jsonb; prior jsonb:=p_audit->'before_data'; later jsonb:=p_audit->'after_data';
 snapshot jsonb; action_value text:=p_audit->>'action';
BEGIN
 IF p_audit->>'entity_type' IS DISTINCT FROM 'operation_schedule_dogs'
   OR p_audit->>'module_code' IS DISTINCT FROM 'operations'
   OR action_value IS NULL OR action_value NOT IN ('created','updated','archived','restored')
 THEN RETURN false; END IF;
 IF coalesce(p_audit->>'entity_id','') !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN RETURN false; END IF;
 SELECT to_jsonb(l) INTO link_json FROM public.operation_schedule_dogs l
 JOIN public.operation_schedules s ON s.id=l.schedule_id
 WHERE l.id=(p_audit->>'entity_id')::uuid;
 IF link_json IS NULL THEN RETURN false; END IF;
 IF action_value='created' THEN
   IF prior IS NOT NULL AND prior<>'null'::jsonb THEN RETURN false; END IF;
 ELSE
   IF jsonb_typeof(prior) IS DISTINCT FROM 'object' THEN RETURN false; END IF;
 END IF;
 IF jsonb_typeof(later) IS DISTINCT FROM 'object' THEN RETURN false; END IF;
 FOR snapshot IN SELECT v FROM (VALUES(prior),(later)) x(v) WHERE v IS NOT NULL AND v<>'null'::jsonb LOOP
   IF NOT (snapshot ?& ARRAY['id','schedule_id','dog_id','archived_at','archive_reason'])
     OR snapshot->>'id' IS DISTINCT FROM link_json->>'id'
     OR snapshot->>'schedule_id' IS DISTINCT FROM link_json->>'schedule_id'
     OR coalesce(snapshot->>'dog_id','') !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
     OR jsonb_typeof(snapshot->'archived_at') NOT IN ('null','string')
     OR jsonb_typeof(snapshot->'archive_reason') NOT IN ('null','string')
   THEN RETURN false; END IF;
   IF NOT EXISTS(SELECT 1 FROM public.dogs d WHERE d.id=(snapshot->>'dog_id')::uuid) THEN RETURN false; END IF;
 END LOOP;
 IF NOT ((prior->>'dog_id'=p_dog_id::text OR later->>'dog_id'=p_dog_id::text) IS TRUE)
 THEN RETURN false; END IF;
 IF action_value='archived' AND NOT (prior->'archived_at'='null'::jsonb AND later->'archived_at'<>'null'::jsonb)
 THEN RETURN false; END IF;
 IF action_value='restored' AND NOT (prior->'archived_at'<>'null'::jsonb AND later->'archived_at'='null'::jsonb)
 THEN RETURN false; END IF;
 IF action_value='updated' AND (prior->'archived_at'='null'::jsonb) IS DISTINCT FROM (later->'archived_at'='null'::jsonb)
 THEN RETURN false; END IF;
 -- A retained current dog or one exact full-snapshot reassignment hop is proof.
 -- Arbitrary nested UUIDs, names, and partial reassignment payloads are not proof.
 IF later->>'dog_id'=link_json->>'dog_id' THEN RETURN true; END IF;
 RETURN EXISTS(SELECT 1 FROM public.entity_audit_events a
   WHERE a.module_code='operations' AND a.entity_type='operation_schedule_dogs'
     AND a.entity_id=(link_json->>'id')::uuid AND a.action='updated'
     AND a.before_data=later AND a.after_data=link_json
     AND (a.before_data->'archived_at'='null'::jsonb)=(a.after_data->'archived_at'='null'::jsonb));
END $$;
REVOKE ALL ON FUNCTION public.dog_schedule_audit_resolved_v2a(jsonb,uuid) FROM PUBLIC,anon,authenticated,service_role;
...
BEGIN
 ...
 SELECT t.* FROM traces t WHERE NOT (
   ...
   (t.source='entity_audit_events' AND (
     ...
     (t.body->>'entity_type' IS DISTINCT FROM 'operation_schedule_dogs' AND t.body->>'entity_id' IN (SELECT key FROM records))
   )) OR
