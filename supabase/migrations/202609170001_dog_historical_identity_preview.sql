-- V2-A READ foundation only. No lifecycle columns, policies, writes or existing function replacements.
BEGIN;

CREATE FUNCTION public.get_historical_dog_identities(p_dog_ids uuid[])
...
BEGIN
  ...
  RETURN (SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    'profileStatus',CASE WHEN d.is_active THEN 'active' ELSE 'inactive' END,
    'canonicalDogId',NULL,'profileReadable',true,
    'customerId',d.customer_id,'breed',d.breed,'sex',d.sex
  ) ORDER BY d.id),'[]'::jsonb) FROM public.dogs d WHERE d.id=ANY(p_dog_ids));
END $$;

-- Structured identity only: never search free-text names/memos or stringify payloads.
CREATE FUNCTION public.dog_identity_in_payload_v2a(p_value jsonb,p_dog_id uuid)
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path=public,pg_temp AS $$
WITH RECURSIVE nodes(value,identity_context) AS (
 SELECT p_value,false
 UNION ALL
 SELECT child.value,
   CASE WHEN jsonb_typeof(n.value)='array' THEN n.identity_context
        ELSE lower(replace(child.key,'_','')) IN
          ('dogid','dogids','bestfrienddogid','recorddogid','sourcedogid','targetdogid')
          OR (n.identity_context AND child.key='id')
          OR lower(child.key) IN ('dog','dogs') END
 FROM nodes n CROSS JOIN LATERAL (
   ...
   UNION ALL
   SELECT NULL,a.value FROM jsonb_array_elements(CASE WHEN jsonb_typeof(n.value)='array' THEN n.value ELSE '[]'::jsonb END) a
 ) child
)
SELECT EXISTS(SELECT 1 FROM nodes WHERE identity_context AND value=to_jsonb(p_dog_id::text))
$$;

-- Inventory-based structured JSON evidence, including future request/receipt tables.
-- Only known identity keys are matched; ordinary master-edit audits remain non-blocking.
CREATE FUNCTION public.dog_structured_traces_v2a(p_dog_id uuid)
RETURNS TABLE(source text,key text,body jsonb)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=public,pg_temp AS $$
DECLARE item record; predicate text;
BEGIN
 FOR item IN
   SELECT c.relname,array_agg(a.attname ORDER BY a.attnum) AS columns
   FROM pg_catalog.pg_class c JOIN pg_catalog.pg_namespace n ON n.oid=c.relnamespace
   JOIN pg_catalog.pg_attribute a ON a.attrelid=c.oid AND a.attnum>0 AND NOT a.attisdropped
   WHERE n.nspname='public' AND c.relkind IN ('r','p') AND a.atttypid IN ('json'::regtype,'jsonb'::regtype)
     AND (c.relname ~ '(audit|request|receipt|history)' OR c.relname IN ('family_bookings','daycare_operation_states'))
   GROUP BY c.relname ORDER BY c.relname
 LOOP
   SELECT string_agg(format('public.dog_identity_in_payload_v2a(t.%I::jsonb,$1)',col),' OR ')
     INTO predicate FROM unnest(item.columns) col;
   RETURN QUERY EXECUTE format(
     'SELECT %L,coalesce(to_jsonb(t)->>''id'',to_jsonb(t)->>''request_id'',to_jsonb(t)->>''operation_schedule_id''),to_jsonb(t)
      FROM public.%I t WHERE (%s) %s',item.relname,item.relname,predicate,
      CASE WHEN item.relname='entity_audit_events' THEN 'AND t.entity_type NOT IN (''dogs'',''dog'')' ELSE '' END
   ) USING p_dog_id;
 END LOOP;
END $$;

CREATE FUNCTION public.preview_dog_profile_removal(p_dog_id uuid)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER
...
BEGIN
 ...
 SELECT t.* FROM traces t WHERE NOT (
   ...
   (t.source='entity_audit_events' AND t.body->>'entity_id' IN (SELECT key FROM records)) OR
...
REVOKE ALL ON FUNCTION public.dog_structured_traces_v2a(uuid) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.dog_identity_in_payload_v2a(jsonb,uuid) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.get_historical_dog_identities(uuid[]) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.preview_dog_profile_removal(uuid) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.get_historical_dog_identities(uuid[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.preview_dog_profile_removal(uuid) TO authenticated;
COMMIT;
