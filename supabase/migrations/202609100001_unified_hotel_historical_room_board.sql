-- 010 actual-use history. New read functions only; 004–009 remain immutable.
BEGIN;
DO $$ BEGIN
 IF md5(btrim((SELECT prosrc FROM pg_proc WHERE oid=to_regprocedure('public.hotel_shared_semantic_internal(text,jsonb)')), E' \t\r\n')) IS DISTINCT FROM '1d7053f3a7a3472d1a00278798d48cf1' THEN RAISE EXCEPTION 'STOP_010_DEPENDENCY_MISMATCH'; END IF;
 IF md5(btrim((SELECT prosrc FROM pg_proc WHERE oid=to_regprocedure('public.hotel_shared_chain_internal(text,jsonb,jsonb)')), E' \t\r\n')) IS DISTINCT FROM 'e444f7370b83e6e93d8f93fc6149fd30' THEN RAISE EXCEPTION 'STOP_010_DEPENDENCY_MISMATCH'; END IF;
 IF md5(btrim((SELECT prosrc FROM pg_proc WHERE oid=to_regprocedure('public.hotel_shared_verified_segments_internal(uuid[])')), E' \t\r\n')) IS DISTINCT FROM '9c255110adb3e596514a2a65a6bd91d6' THEN RAISE EXCEPTION 'STOP_010_DEPENDENCY_MISMATCH'; END IF;
END $$;
...
-- Pure Single/Long Stay resolver. The caller supplies retained rows and complete audit chains.
-- Processing timestamps are deliberately never read here.
CREATE FUNCTION public.hotel_history_individual_010(p jsonb)
RETURNS jsonb LANGUAGE plpgsql IMMUTABLE SET search_path=public,pg_temp AS $$
 ...
 ORDER BY (value->>'allocated_from')::timestamptz,value->>'id' LOOP
  ...
  IF EXISTS(SELECT 1 FROM jsonb_array_elements(audits) e WHERE e->>'entity_type'='hotel_room_allocations' AND e->>'entity_id'=a->>'id' AND e->'after_data' ? 'created_at'
   AND ((e->'after_data'->>'room_id' IS DISTINCT FROM a->>'room_id')
    OR (e->'after_data'->>'allocated_until')::timestamptz > (e->'before_data'->>'allocated_until')::timestamptz
...
-- 008 verification is necessary but not sufficient for 010 actual-use history.
CREATE FUNCTION public.hotel_history_shared_gate_010(p_stay jsonb,p_proof jsonb,p_members jsonb,p_requests jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path=public,pg_temp AS $$
 SELECT coalesce(p_proof->>'resolutionStatus'='resolved' AND jsonb_array_length(p_proof->'segments')=1
 AND jsonb_array_length(p_members)=1
 AND (p_proof->'segments'->0->>'usedFrom')::timestamptz=(p_stay->>'checked_in_at')::timestamptz
 AND (p_stay->>'checked_out_at' IS NULL OR (p_proof->'segments'->0->>'usedUntil')::timestamptz=(p_stay->>'checked_out_at')::timestamptz)
 AND NOT EXISTS(SELECT 1 FROM jsonb_array_elements(p_requests) r WHERE r->>'operation_kind' IN ('move','merge_existing_stays'))
 AND NOT EXISTS(SELECT 1 FROM jsonb_array_elements(p_members) m WHERE m->>'hotel_stay_id' IS DISTINCT FROM p_stay->>'id'
  OR m->>'occupancy_id' IS DISTINCT FROM p_proof->>'physicalOccupancyId'
  OR m->>'joined_at' IS NULL OR (m->>'joined_at')::timestamptz>(p_stay->>'checked_in_at')::timestamptz
  OR (m->>'left_at' IS NOT NULL AND (m->>'left_at')::timestamptz IS DISTINCT FROM (p_stay->>'checked_out_at')::timestamptz)),false);
$$;

CREATE FUNCTION public.get_hotel_historical_room_board(p_local_date date)
 ...
 LOOP
  ...
  ELSE
   evidence:=jsonb_build_object('stay',stay_value,'kind',kind,'capacities',ctx.caps,'allocations',ctx.allocs,'audits',ctx.audit_rows,
    'absences',ctx.absence_rows,'receipts',ctx.receipts,'rooms',rooms,'asOf',observed,'windowUntil',de);
...
END $$;
REVOKE ALL ON FUNCTION public.hotel_history_shared_gate_010(jsonb,jsonb,jsonb,jsonb) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.hotel_history_semantic_010(text,jsonb) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.hotel_history_chain_010(text,jsonb,jsonb) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.hotel_history_individual_010(jsonb) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.get_hotel_historical_room_board(date) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.get_hotel_historical_room_board(date) TO authenticated,service_role;
COMMIT;
