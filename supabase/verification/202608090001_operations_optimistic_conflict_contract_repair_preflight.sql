-- Read-only gate for the Operations optimistic conflict contract repair.
with expected(identity, body_fingerprint, result_type, expected_acl) as (values
  ('public.set_operation_schedule_status(uuid,integer,text,text,uuid)', 'a757e6185ad5576a78864812d4b200ba', 'jsonb', array['authenticated','postgres','service_role']::text[])
), contract as (
...
select case when contract.function_count = 1 and contract.ready and frozen.ready
  then 'READY_TO_APPLY_OPERATIONS_OPTIMISTIC_CONFLICT_CONTRACT_REPAIR'
  else 'STOP_OPERATIONS_OPTIMISTIC_CONFLICT_CONTRACT_REPAIR_PREFLIGHT'
end as status,
