-- Rollback order:
-- 1. Roll back Hotel Flexible Reservations first, if applied.
-- 2. Roll back this reverse-completion repair.
-- 3. Roll back the update_hotel_reservation repair only if required.
...
begin
  ...
  then
    raise exception 'STOP_ROLLBACK_FLEXIBLE_EXTENSION_STILL_APPLIED'
      using detail = 'Flexible Rollback 후 Reverse Completion Repair Rollback을 실행해 주세요.';
  end if;
  ...
  if target_body_fingerprint <> 'dd4dd04865adfa2dc3ec83097e2b81a3' then
