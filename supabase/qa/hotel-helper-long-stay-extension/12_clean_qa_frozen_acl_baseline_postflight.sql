-- CLEAN QA ONLY. Read-only postflight for the Frozen Hotel ACL baseline repair.
...
with expected(
  ...
  select
    ...
    ), false) exact_production_acl_ready
  from target
)
select case
  when not environment_ready then 'STOP_CLEAN_QA_FROZEN_ACL_ENVIRONMENT'
  when not definition_contract_ready then 'STOP_CLEAN_QA_FROZEN_ACL_DEFINITION_DIFF'
  when not exact_production_acl_ready then 'STOP_CLEAN_QA_FROZEN_ACL_NOT_ALIGNED'
  else 'CLEAN_QA_FROZEN_HOTEL_ACL_BASELINE_READY'
