...
begin
  ...
  begin
    result:=public.update_checked_in_hotel_planned_checkout(
      c.stay_id,c.expected_version,date '2099-12-05',time '11:00',false,c.request_b
    );
    insert into hotel_qa.hotel_lifecycle_2session_results
      values('B',true,null,(result->>'version')::integer,started,clock_timestamp());
  exception when others then
    insert into hotel_qa.hotel_lifecycle_2session_results
      values('B',false,sqlstate,null,started,clock_timestamp());
  end;
end;
$session$;
commit;
select 'HOTEL_LIFECYCLE_2SESSION_B_DONE' status;
